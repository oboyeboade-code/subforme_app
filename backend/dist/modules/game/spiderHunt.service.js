...
export const spiderHuntService = {
    startSession: async (userId, difficulty) => {
    ...
    play: async (userId, sessionId, choice, difficulty) => {
    ...
    cashout: async (userId, sessionId) => {
        const session = await SpiderHuntSessionModel.findOne({
            _id: sessionId,
            userId,
            active: true,
        ...
        const res = await coinService.addCoins(userId, earned, "game", {
            game: "spider_hunt",
            sessionId,
        });
...
//# sourceMappingURL=spiderHunt.service.js.map