...
export const userService = {
    ...
    addToWishlist: async (userId, serviceId) => {
        ...
        const result = await CustomerModel.findOneAndUpdate({ _id: userId, 'wishlist.serviceId': { $ne: serviceId } }, {
            $push: {
                wishlist: {
        ...
        }, { new: true, projection: { wishlist: 1 } }).lean();
        if (!result) {
    ...
    removeFromWishlist: async (userId, serviceId) => {
        const result = await CustomerModel.findByIdAndUpdate(userId, { $pull: { wishlist: { serviceId } } }, { new: true, projection: { wishlist: 1 } }).lean();
        if (!result)
            throw new AppError("Customer not found", 404);
        return (result.wishlist || []).map(toIdQty);
    },
    ...
    addToCart: async (userId, serviceId, quantity = 1) => {
        // Check exists & get data for denormalization (Required for Postman-safety)
        const service = await ServiceModel.findById(serviceId).select("name image priceNaira").lean();
        if (!service)
            throw new AppError("Service not found", 404);
        const result = await CustomerModel.updateOne({ _id: userId, "cartItems.serviceId": serviceId }, { $inc: { "cartItems.$.quantity": quantity } });
        if (result.modifiedCount === 0) {
            await CustomerModel.updateOne({ _id: userId }, {
                $push: {
                    cartItems: {
                        serviceId,
                        name: service.name,
    ...
    updateCartItemQuantity: async (userId, serviceId, quantity) => {
        if (!Number.isInteger(quantity) || quantity < 0)
            throw new AppError("Quantity must be a non-negative integer", 400);
        const result = await CustomerModel.findOneAndUpdate({ _id: userId, "cartItems.serviceId": serviceId }, { $set: { "cartItems.$.quantity": quantity } }, { new: true, projection: { "cartItems.serviceId": 1, "cartItems.quantity": 1 } }).lean();
        if (!result) {
            const customerExists = await CustomerModel.exists({ _id: userId });
            if (!customerExists)
                throw new AppError("Customer not found", 404);
