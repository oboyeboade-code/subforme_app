...
import { UserModel, IBaseUser, UserRole } from "../user/user.model.js";
...
export const authService = {
  register: async (data: {
    ...
    const user: IBaseUser = await UserModel.create({
      email,
      password,
      role,
    });
  ...
  me: async (auth: AuthTokenPayload): Promise<MeResponse> => {
  ...
  role: async (auth?: AuthTokenPayload) => {
    return auth?.role;
  },
