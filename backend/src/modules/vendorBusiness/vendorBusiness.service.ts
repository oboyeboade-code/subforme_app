import {
  ...
  OfferModel,
  ServiceCodeModel,
} from "../_shared/user.model.js";

export const VendorService = {
  async getVendorDetails(vendorBusinessId: string) {
    const vendor = await VendorBusinessModel.findById(vendorBusinessId).lean();
    if (!vendor) throw new Error("Vendor not found");
    return vendor;
  },

  async getVendorServices(vendorBusinessId: string) {
    return ServiceModel.find({ vendorBusinessId, isActive: true }).lean();
  },

  async getVendorOffers(vendorBusinessId: string) {
  ...
  async getVendorServiceCodes(vendorBusinessId: string) {
  ...
  async getVendorFull(vendorBusinessId: string) {
   ...
   */
  async getEarnings(vendorBusinessId: string) {
    ...
    const perService = services.map((s) => {
      const codesForService = codes.filter(
        (c) => c.serviceId.toString() === s._id.toString()
      );
    ...
    const totals = perService.reduce(
      (acc, s) => {
