...
export const initPaystack = async (
  ...
  try {
    ...
    if (!secret) throw new AppError("Paystack not configured", 500);
    ...
    const safePath =
      callbackPath && ALLOWED_CALLBACK_PATHS.has(callbackPath)
        ? callbackPath
        : "/app/paid";
    ...
    const payload: any = await paystackRes.json();
...
async function markOrderPaid(
  ...
  if (!existing && metadata) {
    ...
    update.bookingIds = metadata.items?.map((i: any) =>
      new mongoose.Types.ObjectId(i.serviceId)
    );
