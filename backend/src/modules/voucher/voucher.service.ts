...
export const voucherService = {
  buyWithCoins: async (userId: string, valueNaira: number) => {
    if (!mongoose.Types.ObjectId.isValid(userId)) throw new AppError("Invalid user id", 400);
    if (valueNaira <= 0) throw new AppError("Voucher value must be positive", 400);
    ...
    try {
      ...
      const voucher = await VoucherModel.create(
        [{ userId, code, valueNaira, expiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) }],
        { session }
      );
  ...
  validateForCheckout: async (userId: string, code: string, cartTotalNaira: number) => {
    if (!mongoose.Types.ObjectId.isValid(userId)) throw new AppError("Invalid user id", 400);
    ...
    if (!voucher) throw new AppError("Invalid, expired, or already used voucher", 400);
  ...
  consumeAfterPayment: async (userId: string, code: string, orderId: string) => {
    const voucher = await VoucherModel.findOneAndUpdate(
      { userId, code: code.toUpperCase(), status: "active" },
      { status: "used", usedAt: new Date(), orderId },
      { new: true }
    );
    if (!voucher) throw new AppError("Voucher not found or already used", 400);
    return voucher;
  },

  listUserVouchers: async (userId: string) => {
    if (!mongoose.Types.ObjectId.isValid(userId)) throw new AppError("Invalid user id", 400);
