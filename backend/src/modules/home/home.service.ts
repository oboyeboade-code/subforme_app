...
import { IPlatformMetric, TopProvider, AppCategory } from "./home.types.js";

export const homeService = {
  /**
   * FETCH LATEST METRICS
   */
  async getPlatformMetrics(): Promise<IPlatformMetric> {
    const cached = await PlatformMetricModel.findOne()
      .sort({ createdAt: -1 })
      .lean();

    if (cached) {
      return cached as unknown as IPlatformMetric;
    }
   ...
   */
  async syncPlatformMetrics(): Promise<IPlatformMetric> {
    const metrics = await this.computeMetricsLive();
    
    const updated = await PlatformMetricModel.findOneAndUpdate(
      { weekIdentifier: metrics.weekIdentifier },
      { $set: metrics },
      { upsert: true, new: true }
    ).lean();

    return updated as unknown as IPlatformMetric;
  },

  /**
   * INTERNAL: LIVE COMPUTATION LOGIC
   */
  async computeMetricsLive(): Promise<IPlatformMetric> {
   ...
   */
  async getTopProviders(): Promise<TopProvider[]> {
    ...
    return topAgg
      .map((t) => {
        ...
        const serviceNames = t.qualifyingServiceIds
          .map((id: any) => serviceMap.get(id.toString()))
          .filter(Boolean);
